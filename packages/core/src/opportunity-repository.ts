...
export type OrganizationWriteInput = Omit<OrganizationInput, 'normalizedName'> & {
...
function immutableSourceContent(source: OpportunitySource): string {
...
export class OpportunityRepository {
  ...
  private source(
    ...
    return row ? mapOpportunitySource(row) : null;
  }

  upsertOrganization(input: OrganizationWriteInput): Organization {
    const name = input.name.trim();
    const value = OrganizationSchema.parse({
      ...input,
      name,
      normalizedName: input.normalizedName?.trim() || normalizedName(name),
    });
  ...
  attachSource(input: OpportunitySourceInput): OpportunitySource {
    ...
    const existing = this.source(value.opportunityId, value.sourceId, value.sourceRole);
    if (existing && existing.reviewState !== 'pending') {
      if (
        immutableSourceContent(existing) === immutableSourceContent(value) &&
        existing.reviewState === value.reviewState &&
        existing.reviewedAt === value.reviewedAt
      ) {
        return existing;
      }
    ...
    return this.source(value.opportunityId, value.sourceId, value.sourceRole)!;
  }
  ...
  reviewSource(
    ...
    const current = this.source(opportunityIdInput, sourceIdInput, sourceRoleInput);
    if (!current) throw new Error('Opportunity evidence does not exist');
    ...
    return this.source(current.opportunityId, current.sourceId, current.sourceRole)!;
  }
  ...
  listOpportunities(filter: OpportunityFilter = {}): Opportunity[] {
    ...
    if (filter.deadlineBefore) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/u.test(deadline)) {
        throw new Error('deadlineBefore must use YYYY-MM-DD');
      }
