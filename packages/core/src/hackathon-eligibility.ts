...
function monthsBetween(founderDate: string, evaluatedAt: string): number | null {
  const founded = new Date(`${founderDate}T00:00:00.000Z`);
...
function unreviewedDetail(rule: HackathonRule): EligibilityRuleDetail | null {
...
function evaluateRule(profile: EligibilityProfile, rule: HackathonRule): EligibilityRuleDetail {
  const reviewFailure = unreviewedDetail(rule);
  ...
  switch (rule.ruleType) {
    case 'geography': {
      ...
      if (!allowed && !prohibited) {
        return detail(rule, 'uncertain', 'The geography rule has no allowed or prohibited list.');
      }
    ...
    case 'student_status': {
      ...
      if (required && !profile.isStudent) {
        return detail(rule, 'ineligible', 'This opportunity requires a current student.');
      }
      return detail(rule, 'eligible', 'Student status satisfies the reviewed rule.');
    }
    case 'company_age': {
    ...
    case 'open_source': {
      ...
      if (required && !profile.willOpenSource) {
        return detail(rule, 'ineligible', 'The reviewed rules require an open-source submission.');
      }
      return detail(rule, 'eligible', 'Open-source commitment satisfies the reviewed rule.');
    }
    ...
    case 'attendance': {
      ...
      if (required === 'in_person' && profile.canAttendInPerson !== true) {
        return detail(rule, 'ineligible', 'The event requires in-person attendance.');
      }
      if (required === 'online' && profile.attendanceMode === 'in_person') {
        return detail(rule, 'ineligible', 'The selected attendance mode is not online.');
      }
      if (required === 'hybrid' && profile.attendanceMode === 'unknown') {
        return detail(rule, 'uncertain', 'Hybrid attendance capability is unknown.');
      }
      return detail(rule, 'eligible', 'Attendance capability satisfies the reviewed rule.');
    }
    case 'prior_funding': {
...
function rulesDigest(rules: readonly HackathonRule[]): string {
...
export function evaluateHackathonEligibility(
  ...
  return {
    ...
    rulesSnapshotSha256: rulesDigest(rules),
    details,
  };
}
