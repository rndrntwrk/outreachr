...
describe('hackathon entry readiness and state transitions', () => {
  ...
  it('does not treat a missing asset list as submission-ready', () => {
    const readiness = calculateHackathonReadiness(readinessInput({ requiredAssets: [] }));
    expect(readiness.assetsReady).toBe(false);
    expect(readiness.readyForSubmission).toBe(false);
  });
});
