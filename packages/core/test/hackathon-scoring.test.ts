...
describe('hackathon portfolio scoring', () => {
  it('uses the checked-in founder policy and returns a deterministic one-decimal score', () => {
  ...
  it('recommends go only when score, reuse, effort, authority, eligibility and time all pass', () => {
    expect(evaluateHackathonGoNoGo(decisionInput())).toEqual({
      recommendation: 'go',
      weightedScore: 83.7,
      blockingReasons: [],
      conditions: [],
  ...
  it('recommends no-go for low reuse even when the weighted score is competitive', () => {
  ...
  it('allows conditional-go only with explicit founder conditions', () => {
  ...
  it('rejects invalid ratings and dates instead of silently normalizing them', () => {
    expect(() => calculateHackathonScore(decisionInput({ strategicFit: 0 }))).toThrow(
      'strategicFit must be between 1 and 10',
    );
    expect(() => calculateHackathonScore(decisionInput({ deadline: 'not-a-date' }))).toThrow(
      'deadline must be null or a valid date-time',
    );
  });
});
